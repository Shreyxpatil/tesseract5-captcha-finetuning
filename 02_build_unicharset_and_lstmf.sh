...
for tif_file in "$TRAIN_DIR"/*.tif; do
    ...
    if TESSDATA_PREFIX="$TESSDATA" tesseract \
            "$tif_file" "$out_prefix" \
            --psm 13 \
            lstm.train \
            >/dev/null 2>&1; then
...
all_file = pathlib.Path("/home/ca/Projects/captcha_model/model_output/all_lstmf.txt")
...
out = pathlib.Path("/home/ca/Projects/captcha_model/model_output")
